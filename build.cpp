...
static void gen_phone(int64_t ind, char *target, long seed) {
 ...
 */
void mk_sparse(int64_t i, int64_t *ok, long seq) {
  long low_bits;
...
long mk_time(int64_t index, dss_time_t *t) {
  long m = 0;
  long y;
  long d;
...
int mk_nation(int64_t index, code_t *c) {
  c->code = index - 1;
...
int mk_region(int64_t index, code_t *c) {
